...
iii. Choice of chain length, $n$.  Longer chains give more a convergent estimate of $\pi$, i.e. low values of $n$ give variables estimates, which makes sense given the law of of large numbers.  Also, shorter chains do not have chance to explore the ample space as much as longer chains do.
...
d. The above overestimates of $\pi$ regardless of parameters.  The chain must be updated to take account of the M-H ratio, given by:
...
random_walker <- function(w1_0, w2_0, epsilon, n){
  ...
  for (i in c(1:n) ){
    ...
    # compute MH ratio
    f_y <- dunif(w1_move, -1 , 1)*dunif(w2_move, -1 , 1)
    f_x <- dunif(w1_t[i], -1 , 1)*dunif(w2_t[i], -1 , 1)
    g_y <- dunif(w1_move, max(-1, w1_t[i]-epsilon) , min(1, w1_t[i]+epsilon))*
      dunif(w2_move, max(-1, w2_t[i]-epsilon) , min(1, w2_t[i]+epsilon))    
    g_x <- dunif(w1_t[i], max(-1, w1_t[i]-epsilon) , min(1, w1_t[i]+epsilon))*
      dunif(w2_t[i], max(-1, w2_t[i]-epsilon) , min(1, w2_t[i]+epsilon))

    r <- (f_y*g_x)/(f_x*g_y)
    # Move according to MH ratio
    w1_t[i+1] <- w1_t[i]
    w2_t[i+1] <- w2_t[i]    
    if (u < r){ 
      w1_t[i+1] <- w1_move
