...
e. THIS QUESTION IS ACTUALLY NOT POSSIBLE WITH GIVEN PRIOR.


For the prior, want $\sigma^2=4$, $P(\sigma^2 > 2 )=0.9$.

So that $\frac{\alpha_0 - 1}{\beta_0} = 4$, $pbeta(2, \alpha_0, \beta_0) = 1- 0.9 = 0.1$
```{r}
x <- 2

# start with 5 going to 1 to ensure alpha_0, beta_0 > 0
#vector <- rev(seq(1, 5, by = 0.001))
vector <- seq(1, 5, by = 0.00001)
i <- 1
...
epsilson <- 0.001
while (abs(pgamma(x, alpha_0, beta_0) - 0.1) > epsilson){
  #print(i)
...
alpha_0
beta_0
pgamma(x, alpha_0, beta_0) # roughly = 0.1
```
