...
First suppose that all draws are the same, so $\hat{p}_i = $ mean$($Beta$(\hat{\alpha},\hat{\beta}))$.
...
Second suppose that all draws are different, so $\hat{p}_i = y_i \sim $Beta$(\hat{\alpha},\hat{\beta}))$.
...
Let's show how the same approach estimates $\alpha_0$ and $\ebta_0$ in my smaller data set.
